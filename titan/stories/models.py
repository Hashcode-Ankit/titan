...
from titan.models import BaseModel, ModelWithProgress
...
class Story(BaseModel):
    ...
    requester = models.ForeignKey(
        settings.AUTH_USER_MODEL, null=True, blank=True, on_delete=models.SET_NULL, related_name="requested_tasks"
    )
...
def handle_story_pre_save(sender, **kwargs):
    if not kwargs.get("raw", False):
        ...
        if (previous_sprint != instance.sprint) and previous_sprint is not None:
            from titan.sprints.tasks import handle_sprint_change
